...
class QuanlytimvieclamController extends Controller
{
    ...
    public function chitietvieclam($id)
    {
        ...
        return view('vieclam.chitietvieclam',compact('khuvuc','khuvuc_n','nganhnghe_n','nganhnghe','vieclam','hethang','ungtuyen','luu'));
    }
        ...
        $this->validate($req,
            ...
            'g-recaptcha-response' => function ($attribute, $value, $fail){
                ...
                if (!$response->success){
                 ...
                 $fail($attribute.'google recCaptcha failed');
                }
            }
            ],
            [
    ...
    public function capnhaphosoungvien(Request $req)
    {
        $this->validate($req,
            ...
            $anhgiayto='';
            if ($req->hasfile('giayto')) 
            {  $this->validate($req, 
                //hàm kiểm tra dữ liệu
                [
                'anhgiayto' => 'mimes:jpg,png,jpeg,gif|max:2048',
                ],
                [
                'anhgiayto.mimes' => ' Chỉ chấp nhận ảnh với đuôi .jpg .jpeg .png .gif',
                'anhgiayto.max' => 'Hình ảnh giới hạn dung lượng không quá 2M',
                ]);
                $file = $req->file('giayto');
                $anhgiayto = $file->getClientOriginalName();
                $destinationPath = public_path('static\assets\images\giayto');
                $file->move($destinationPath, $anhgiayto);
           
        }; 
        /*$ungvien = Ungvien::find($id_user);
        if ($anhgiayto=="") {
            $anhgiayto=$ungvien->anhgiayto;
        }*/
        ...
        $ungvien->anhgiayto = $req->anhgiayto;
        
        $ungvien->save();
        return redirect()->back()->with('success','Đã cập nhập hồ sơ');
    }
  /* public function postcapnhaphosoungvien(Request $req){
            ...
            'kinhnghiem'=>'required|integer',
            'diachi'=>'required|regex:/(^[\pL0-9 ]+$)/u',
            'luonghientai'=>'required|integer',
            'luongmongmuon'=>'required|integer',
            'vitri'=>'required',
            'loaihinh'=>'required',
            'bangcap'=>'required',
            'khuvuc'=>'required',
            ...
            'loaihinh.required'=>'Vui lòng chọn',
            ...
            $anhgiayto='';
            if ($req->hasfile('anhgiayto')) 
            {

            $file = $req->file('anhgiayto');
            $anhgiayto = $file->getClientOriginalName();
            $destinationPath = public_path('static\assets\images\avt');
            $file->move($destinationPath, $anhgiayto);
        };
        ...
        $ungvien->sdt = $req->sdt;
        $ungvien->diachi = $req->diachi;
        $ungvien->kinhnghiem = $req->inputkinhnghiem;
        $ungvien->luonghientai = $req->luonghientai;
        $ungvien->luongmongmuon = $req->luongmongmuon;
        $ungvien->vitri = $req->vitri;
        $ungvien->loaihinh = $req->loaihinh;
    ...
    public function dangbaimoiNTD(Request $req)
    {
        ...
        [
            ...
            'bangcap.required'=>'Vui lòng chọn',
            'giayto'=>'Giấy tờ không được nhập ký tự đặc biệt',
            'gioitinh.required'=>'Vui lòng chọn',
            'vitri.required'=>'Vui lòng chọn',
        ...
        $vieclam->giayto = $req->input('giayto');
    ...
    public function postsuaVL(Request $req,$id)
    {
        ...
        'giayto.required'=>'Vui lòng nhập giấy tờ yêu cầu',
